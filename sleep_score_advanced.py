...
# Generate rolling baselines for HRV and resting heart rate
df_clean["date"] = pd.to_datetime(df_clean["date"])
df_clean = df_clean.sort_values(by=["user_id","date"]).reset_index(drop=True)

window_size = 7
df_clean["rolling_hrv_baseline"] = df_clean.groupby("user_id")["hrv"].transform(
    lambda x: x.shift(1).rolling(window=window_size, min_periods=1).mean())
df_clean["rolling_rhr_baseline"] = df_clean.groupby("user_id")["resting_heart_rate"].transform(
    lambda x: x.shift(1).rolling(window=window_size, min_periods=1).mean())
df_clean["rolling_hrv_baseline"] = df_clean["rolling_hrv_baseline"].fillna(df_clean["hrv_baseline"])
df_clean["rolling_rhr_baseline"] = df_clean["rolling_rhr_baseline"].fillna(df_clean["rhr_baseline"])
...
def score3(row):
    ...
    hrv = row["hrv"]
    hrv_base = row["rolling_hrv_baseline"]
    ...
    rhr_base = row["rolling_rhr_baseline"]
...
# Calculate the raw sleep score
def calculate_sleep_score(row):
    return score1(row) * 0.3 + score2(row) * 0.25 + score3(row) * 0.3 + score4(row) * 0.15

print("Calculating scores...")
df_clean["raw_sleep_score"] = df_clean.apply(calculate_sleep_score, axis=1)

# Z-score adjustment
df_clean["score_mean"] = df_clean.groupby("user_id")["raw_sleep_score"].transform(
    lambda x: x.shift(1).rolling(window=window_size, min_periods=3).mean())
df_clean["score_std"] = df_clean.groupby("user_id")["raw_sleep_score"].transform(
    lambda x: x.shift(1).rolling(window=window_size, min_periods=3).std())
df_clean["score_mean"] = df_clean["score_mean"].fillna(df_clean["raw_sleep_score"].mean())
df_clean["score_std"] = df_clean["score_std"].fillna(df_clean["raw_sleep_score"].std())
df_clean["score_std"] = df_clean["score_std"].replace(0, 1)

def normalize_score_with_zscore(row):
    z = (row["raw_sleep_score"] - row["score_mean"]) / row["score_std"]
    k = 1
    bias = 1.5
    z_normalized_score = 100 * (1 / (1 + np.exp(-(k * z + bias))))
    return z_normalized_score

df_clean["z_normalized_score"] = df_clean.apply(normalize_score_with_zscore, axis=1)
...
df_clean["trend_slope"] = df_clean.groupby("user_id")["raw_sleep_score"].transform(
    lambda x: x.rolling(window=7, min_periods=3).apply(calculate_trend_slope, raw=True)).fillna(0)

def slope_to_score(slope, sensitivity=4):
    normalized = np.tanh(slope / sensitivity)
    score = 80 + (25 * normalized)
    return np.clip(score, 0, 100)

df_clean["trend_score"] = df_clean["trend_slope"].apply(lambda x: slope_to_score(x, sensitivity=4))

# Calculate the final sleep score
weight_state = 0.75
weight_trend = 0.25
df_clean["final_sleep_score"] = df_clean["z_normalized_score"] * weight_state + df_clean["trend_score"] * weight_trend
...
# Explore the correlation between recovery scores and final sleep scores
print("="*60)
...
# Advanced correlation analysis: Daily recovery score changes vs. final sleep scores
